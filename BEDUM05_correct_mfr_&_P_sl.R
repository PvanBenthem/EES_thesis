...
# 
# tmp <- datP
# tmp$H_i <- (tmp$mfr/1000) * 4.182 * T_in 
# tmp$H_p <- (tmp$mfr/1000) * 4.182 * tmp$Tp
# 
# tmp$h_i <- tmp$H_i/tmp$mfr
# tmp$h_p <- tmp$H_p/tmp$mfr
# 
# tmp$Pnet <- tmp$mfr * (tmp$h_p - tmp$h_i) * 0.11
# 
# ggplot(tmp, aes(x=mfr, y=Pnet)) + geom_line()

##########plot for both mfrs for first and second loop 

datp6 <- datP$mfr
datp6 <- as.data.frame(datp6)
datp6 <- datp6 %>%
  rename(mfr = datp6) 
datp6$Tp2 <- datP$Tp2 -273
...
datp6 <- datp6 %>%
  rename(mfr = mfr, "First" = Tp, "Second" = Tp2) %>%
  gather(key = 'Loop', value = "value", -mfr)

p6 <- ggplot(datp6, aes(x=mfr, y=value)) +
  geom_line(aes(color = Loop),size = 1.3)+ #)aes(color = variable)) + 
  scale_color_manual(values = c("steelblue","darkred")) +
  # geom_line(fill = NA) +
  #scale_y_reverse() + 
  #ggtitle("Production 1 and 2 loops") +
  xlab("Mass Flow rate (kg/s)") + ylab("Temperature (°C)") +
  # scale_x_continuous(breaks = scales::pretty_breaks(n = 10), position = "top")  +
  theme_bw(base_size = 16) +
  theme(text=element_text(size=30)) +
  theme(axis.title.x = element_text(margin = margin(t = 10, r =0 , b = 0, l = 0))) +
