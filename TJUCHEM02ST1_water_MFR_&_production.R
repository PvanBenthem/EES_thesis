...
dat1$avg_cp <- mean(dat1$cp)
dat1$avg_lambda <- mean(dat1$lambda)
dat1$avg_ro <- mean(dat1$ro)

datP <- data.frame(1:6010)
datP <- datP %>%
  rename(z=X1.6010)
...
#Mass flow rate at outflow of well: density * velocity* Area ()
datP$mf_out <- (dat2$ro/1000) * datP$V * dat2$dh_p
#Energy within the fluid (outflow temperature - input temperature): cp*(T_out-T-in)
datP$E_1 <- dat3$Cp[1] * (datP$Tp - T_in)
datP$E_2 <- dat3$Cp[1] * (datP$Tp2 - T_in)
#Power output: mass flow rate * energy
datP$P_1 <- datP$mf_out * datP$E_1
datP$P_2 <- datP$mf_out * datP$E_2
...
p4 <- ggplot(datp4, aes(x=mfr, y = Tp)) +
  ...
  geom_vline(xintercept = 1.411090, linetype="dotted",  color = "darkred", size=1) +
...
p5 <- ggplot(datp5, aes(x=mfr, y=value)) +
  geom_line(aes(color = variable))+ #)aes(color = variable)) + 
  scale_color_manual(values = c("steelblue","darkred")) +
  # geom_line(fill = NA) +
  #scale_y_reverse() + 
  ggtitle("Production 1 and 2 loops") +
  xlab("mfr (kg/s)") + ylab("Production (kW)") +
  # scale_x_continuous(breaks = scales::pretty_breaks(n = 10), position = "top")  +
  theme_bw(base_size = 16)
...
datp6$P_1 <- datP$P_1 / 1000
datp6$Tp <- datP$Tp - 273

scale = 16


p6 <- ggplot(datp6, aes(x = mfr, y = Tp)) +
  geom_line(aes(color = "Outlet temperature fluid"),size = 1.3) +
  geom_line(aes(y = P_1/scale, color = "Thermal power"),size = 1.3) +
  scale_x_continuous(breaks = seq(0, 10, 2)) +
  scale_y_continuous(sec.axis = sec_axis(~.*scale, name="Thermal power (kW)")) +
  labs(x = "Mass Flow rate (kg/s)", y = "Temperature (°C)", color = "") +
  scale_color_manual(values = c("#0072B2","#D55E00")) +
  theme_bw(base_size = 16) + 
  theme(text=element_text(size=30)) +
  theme(axis.title.x = element_text(margin = margin(t = 10, r =0 , b = 0, l = 0))) +
  theme(axis.title.y = element_text(margin = margin(t = 0, r =20 , b = 0, l = 0))) +
  theme(axis.title.y.right = element_text(margin =margin(t = 0, r = 0, b = 10, l = 20)))
